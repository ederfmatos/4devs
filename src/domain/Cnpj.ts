export class Cnpj {
    ...
    validateWithDetails(): {
        ...
        if (has14Digits && !allSame) {
            ...
            if (!firstDigitValid) {
                errors.push(`Primeiro dígito verificador incorreto. Esperado: ${expectedFirstDigit}, Encontrado: ${digits[12]}`)
            }

            if (firstDigitValid) {
                const expectedSecondDigit = Cnpj.calculateSecondDigit(digits.slice(0, 13))
                ...
                if (!secondDigitValid) {
                    errors.push(`Segundo dígito verificador incorreto. Esperado: ${expectedSecondDigit}, Encontrado: ${digits[13]}`)
                }
            }
        }
