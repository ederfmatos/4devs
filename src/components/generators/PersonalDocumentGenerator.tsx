...
const PersonalDocumentGenerator = () => {
  ...
  const documentTypes = [
    { value: 'pis', label: 'PIS/PASEP/NIS', mask: '###.#####.##-#' },
    {
      value: 'titulo-eleitor',
      label: 'Título de Eleitor',
      mask: '#### #### ####',
    },
    { value: 'passaporte', label: 'Passaporte Brasileiro', mask: 'AA ######' },
  ];
  ...
  const generateSingleDocument = () => {
    ...
    switch (documentType) {
      case 'pis':
        document = Pis.generate().format();
        break;
      case 'titulo-eleitor':
        document = TituloEleitor.generate().format();
        break;
      case 'passaporte':
        document = Passaporte.generate().format();
        break;
      default:
        document = '';
    }
  ...
  const generateMultipleDocuments = () => {
    ...
    switch (documentType) {
      case 'pis':
        documents = Pis.generateMultiple(quantity).map(doc => doc.format());
        break;
      case 'titulo-eleitor':
        documents = TituloEleitor.generateMultiple(quantity).map(doc =>
          doc.format(),
        );
        break;
      case 'passaporte':
        documents = Passaporte.generateMultiple(quantity).map(doc =>
          doc.format(),
        );
        break;
    }
    ...
    <div className='space-y-6'>
      <div className='text-center'>
        <Text variant='h2' weight='bold' className='mb-2'>
          Gerador de Documentos Pessoais
        </Text>
        <Text variant='body-lg' color='secondary'>
          Gere PIS/PASEP/NIS, Título de Eleitor e Passaporte Brasileiro válidos
        </Text>
      </div>
          ...
          <div>
            ...
            <div className='space-y-1'>
              <Text variant='body-sm' color='info'>
                • <strong>PIS/PASEP/NIS:</strong> Gerados com dígito verificador
                válido
              </Text>
              <Text variant='body-sm' color='info'>
                • <strong>Título de Eleitor:</strong> Números válidos com estado
                aleatório
              </Text>
              <Text variant='body-sm' color='info'>
                • <strong>Passaporte:</strong> Formato brasileiro com série e
                número
              </Text>
              <Text variant='body-sm' color='info'>
...
export default PersonalDocumentGenerator;
