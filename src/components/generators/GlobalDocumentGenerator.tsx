...
import { CartaoSus, Iban, Imei, Isbn, Issn } from '@/domain';
...
const GlobalDocumentGenerator = () => {
  ...
  const documentTypes = [
    { value: 'iban', label: 'IBAN', mask: 'AA## #### #### #### #### #### #' },
    { value: 'isbn10', label: 'ISBN-10', mask: '#-##-######-#' },
    { value: 'isbn13', label: 'ISBN-13', mask: '###-##-####-##-#' },
    { value: 'issn', label: 'ISSN', mask: '####-####' },
    { value: 'sus', label: 'Cartão SUS', mask: '### #### #### ###' },
    { value: 'imei', label: 'IMEI', mask: '## ###### ###### #' },
  ];
  ...
  const generateSingleDocument = () => {
    ...
    switch (documentType) {
      case 'iban':
        document = Iban.generate('BR').format();
        break;
      case 'isbn10':
        document = Isbn.generate('isbn10').format();
        break;
      case 'isbn13':
        document = Isbn.generate('isbn13').format();
        break;
      case 'issn':
        document = Issn.generate().format();
        break;
      case 'sus':
        document = CartaoSus.generate().format();
        break;
      case 'imei':
        document = Imei.generate().format();
        break;
      default:
        document = '';
    }
  ...
  const generateMultipleDocuments = () => {
    ...
    switch (documentType) {
      case 'iban':
        documents = Iban.generateMultiple(quantity, 'BR').map(doc =>
          doc.format(),
        );
        break;
      case 'isbn10':
        documents = Isbn.generateMultiple(quantity, 'isbn10').map(doc =>
          doc.format(),
        );
        break;
      case 'isbn13':
        documents = Isbn.generateMultiple(quantity, 'isbn13').map(doc =>
          doc.format(),
        );
        break;
      case 'issn':
        documents = Issn.generateMultiple(quantity).map(doc => doc.format());
        break;
      case 'sus':
        documents = CartaoSus.generateMultiple(quantity).map(doc =>
          doc.format(),
        );
        break;
      case 'imei':
        documents = Imei.generateMultiple(quantity).map(doc => doc.format());
        break;
    }
    ...
    <div className='space-y-6'>
      <div className='text-center'>
        <Text variant='h2' weight='bold' className='mb-2'>
          Gerador de Documentos Globais
        </Text>
        <Text variant='body-lg' color='secondary'>
          Gere IBAN, ISBN, ISSN, Cartão SUS e IMEI válidos
        </Text>
      </div>
...
export default GlobalDocumentGenerator;
